...
class TestCharm(unittest.TestCase):
    ...
    def test_config_changed_with_agent(self):
        ...
        self.harness.charm._agent_filename = os.path.join(
            self.tempdir.name, "no-such-dir", "agent.json"
        )
        self.harness.update_config(
            {
                "candid-agent-username": "username@candid",
    ...
    def test_vault_relation_joined(self):
        ...
        self.harness.model.get_binding().network.egress_subnets[
            0
        ].network_address = ipaddress.IPv4Address("127.0.0.253")
    ...
    def test_vault_relation_changed(self):
        self.harness.charm._vault_secret_filename = os.path.join(
            self.tempdir.name, "vault.json"
        )
        self.harness.model.get_binding = MagicMock()
        self.harness.model.get_binding().network.egress_subnets[
            0
    ...
    def test_update_status(self):
        self.harness.charm._workload_filename = os.path.join(
            self.tempdir.name, "jimm.bin"
        )
