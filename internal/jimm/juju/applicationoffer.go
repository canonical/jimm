...
func (j *JujuManager) Offer(ctx context.Context, user *openfga.User, offer AddApplicationOfferParams) error {
	...
	if err == nil {
		...
		checkAPI, dialErr := j.dialControllerAsUser(ctx, user, &model.Controller, model.ResourceTag())
		if dialErr != nil {
			return dialErr
		}
...
func (j *JujuManager) GetApplicationOfferConsumeDetails(ctx context.Context, user *openfga.User, details *jujuparams.ConsumeOfferDetails, v bakery.Version) error {
	...
	api, err := j.dialControllerAsUser(ctx, user, &offer.Model.Controller, offer.Model.ResourceTag(), offer.ResourceTag())
	if err != nil {
		return err
	}
	defer api.Close()
