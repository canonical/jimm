...
func TestFindAuditEvents(t *testing.T) {
	...
	admin := openfga.NewUser(alice, j.OpenFGAClient)
	...
	privileged := openfga.NewUser(bob, j.OpenFGAClient)
	...
	c.Assert(err, qt.IsNil)
	unprivileged := openfga.NewUser(eve, j.OpenFGAClient)
