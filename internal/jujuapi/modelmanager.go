...
func (r *controllerRoot) CreateModel(ctx context.Context, args jujuparams.ModelCreateArgs) (jujuparams.ModelInfo, error) {
	...
	mcd := jimm.ModelCreateDefaults{
		DefaultCloud: func() (names.CloudTag, error) { return r.jimm.DefaultCloud(ctx, r.user) },
	}
	if err := mca.FromJujuModelCreateArgs(&args, mcd); err != nil {
